...
namespace ElementPrinting
{
    public partial class Form1 : Form
    {
        ...
        Pen penForLines = new Pen(Color.AliceBlue);
        ...
        void FitnessStagnation() //Неизменность F на протяжении опр-го числа поколений
        {
            ...
            ListOfParentalSpecies.AddRange(population.GenerateInitialPopulation(elementSize.Matrix.Length / 3));
        ...
        private void GenerateNewGeneration(Population population)
        {
            ...
            ListOfOffspringSpecies.AddRange(population.Crossover(elementSize.Matrix.Length / 3, ListOfParentalSpecies));
            ListOfOffspringSpeciesMutated.AddRange(population.Mutation(elementSize.Matrix.Length / 3, ListOfOffspringSpecies, Mutation));
            ...
            foreach (var item in ListOfSpeciesUNITED)
            {
                ...
                item.Fitness = fitness.Count(placement.Elements, adjacencyExtra.Matrix, heat.Matrix, placement.ElementsExtra);
        ...
        private void DrawElements() //Метод, рисющий элементы схемы
        {
            ...
            graphics.DrawRectangle(penForLines, 0, 0, (float)PCBWidth, (float)PCBHeight);
            colorForText = Color.FromArgb(255, 255, 255);
            foreach (var item in placement.Elements)
            {
                ...
                graphics.DrawString($"{index}", new Font("Arial", 8), brushForText, (float)(item.position.x + item.width / 2 - Font.Size / 2), (float)(item.position.y + item.height / 2 - Font.Height / 2));
            ...
            foreach (var item in placement.ElementsExtra)
            {
                ...
                if ((X > item.position.x) && (X < item.position.x + item.width) && (Y > item.position.y) && (Y < item.position.y + item.height))
                {
                    ...
                    for (int j = 0; j < Math.Sqrt(adjacency.Matrix.Length); j++)
                    {
                        ...
                        {
                            graphics.DrawLine(penForLines, (float)placement.ElementsExtra[index].position.x + (float)placement.ElementsExtra[index].width / 2, (float)placement.ElementsExtra[index].position.y + (float)placement.ElementsExtra[index].height / 2, (float)placement.ElementsExtra[j].position.x + (float)placement.ElementsExtra[j].width / 2, (float)placement.ElementsExtra[j].position.y + (float)placement.ElementsExtra[j].height / 2);
                        }
                    }
        ...
        private void PrintResult()
        {
            ...
            ProgramResultLABEL.Text += "\n";
            ProgramResultLABEL.ForeColor = Color.FromArgb(255, 255, 255);
            foreach (var gene in BestSpeciesSorted[0].Genes)
            {
                ProgramResultLABEL.Text += gene;
                ProgramResultLABEL.Text += " \t";
            }
            ProgramResultLABEL.Text += "\t \t FITNESS = ";
            ProgramResultLABEL.Text += BestSpeciesSorted[0].Fitness;
